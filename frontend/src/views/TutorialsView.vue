...
<template>
  <section class="mx-auto max-w-[1120px] pb-12" aria-labelledby="tutorial-heading">
    <header class="border-b border-divider/20 pb-7">
      <h1 id="tutorial-heading" class="font-headline text-3xl font-extrabold">튜토리얼</h1>
    ...
    <div class="mt-6 grid gap-3 sm:grid-cols-3" aria-label="튜토리얼 필터">
      <select
        v-model="type"
        aria-label="튜토리얼 유형"
        class="rounded-lg border border-divider/20 bg-white px-3 py-2 text-sm"
      >
    ...
    <p v-else-if="tutorials.length === 0" class="py-16 text-center text-sm text-body-light">
      조건에 맞는 튜토리얼이 없습니다.
    </p>
    <div v-else class="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
      <article
        ...
        :key="tutorial.id"
        class="flex flex-col rounded-xl border border-divider/20 bg-white p-5"
      >
        ...
        <h2 class="mt-3 font-headline text-lg font-bold">{{ tutorial.title }}</h2>
        <p class="mt-2 flex-1 text-sm leading-6 text-body-light">{{ tutorial.description }}</p>
