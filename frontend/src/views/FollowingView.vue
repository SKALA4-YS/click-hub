...
<template>
  ...
  <section v-else class="mx-auto max-w-[1120px] pb-14" aria-labelledby="following-heading">
    <header class="flex flex-wrap items-end justify-between gap-5">
      <div>
        <h1 id="following-heading" class="font-headline text-3xl font-extrabold">내 팔로잉 관리</h1>
        <p class="mt-2 text-sm text-body-light">구독한 메이커와 공개 프로젝트 수를 확인합니다.</p>
      </div>
      <strong class="rounded-xl border border-divider/20 bg-white px-5 py-3"
    ...
    >
      ...
      <input v-model="query" class="w-full outline-none" placeholder="메이커 이름 검색..." />
    </label>
    ...
    <p v-else-if="filteredFollowing.length === 0" class="py-16 text-center text-sm text-body-light">
      구독 중인 메이커가 없습니다.
    </p>
    <div v-else class="mt-6 space-y-4">
      <article
        ...
        class="flex flex-wrap items-center gap-4 rounded-2xl border border-divider/20 bg-white p-5"
      >
        <img
        ...
        <span
          v-else
          class="grid h-12 w-12 place-items-center rounded-full bg-primary-50 font-bold text-primary-700"
        ...
        <div class="min-w-0 flex-1">
          ...
          >
          <p class="mt-1 text-xs text-body-light">
